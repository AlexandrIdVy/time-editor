...
void main(array<String^>^ arg) {
	...
	RT::RTForm form; //WinFormsTest - им€ вашего проекта
	Application::Run(% form);	
}

// функци€ дл€ подсчета суммы кодов символов ASCII
...
// функци€ дл€ извлечени€ посдтроки со временем
...
int poiskIzamena(std::string fileName, int numberStrTimeSearch, std::string hh, std::string mm, std::string ss, int numberStrTimeReplace)
{
	...
	sumNewTime = sumKodAscii(new_time);
		...
		// замен€ем в строке textline старое врем€ time на новое new_time
	...
	// открываем файл дл€ изменение первой строки
	...
	std::fstream::pos_type posTime = fon.tellg();
		...
		getline(fon, oldTimeTextline);
			...
			{
				// возвращаемс€ к началу строки и мен€ем ее
				fon.seekp(posTime);
				fon << newTimeTextline;
			}
	fon.close();
				...
				if (!finn) { finn.close(); return 0; }
					...
					// вставл€ем фразу "" в начало строки
